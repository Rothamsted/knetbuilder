...
@DataURL(name = "KEGG databases", description = "KEGG genes, ligand, pathway, and brite databases. keggHierarchy file is optional and adds a shallow hierarchy of super pathways.", urls = {
		"ftp://ftp.genome.jp/pub/kegg/release/current/brite.tar.gz",
...
@Authors(authors = { "Matthew Hindle", "Jan Taubert" }, emails = {
...
public class Parser extends ONDEXParser implements ArgumentNames {
	 ...
	 */
	private static Set<GenomeParser.Taxonomony> parseTaxids(
			List<String> speciesNames, GenomeParser genomeParser) {
		Set<GenomeParser.Taxonomony> orgs = new HashSet<GenomeParser.Taxonomony>(
				15);

		for (String speciesName : speciesNames) {
			try {
				GenomeParser.Taxonomony taxid = genomeParser
						.getTaxonomony(Integer.parseInt(speciesName));
				if (taxid == null)
					System.err.println("Unknown ncbi taxid " + speciesName
							+ " in parseTaxids() of Parser ");
				else
					orgs.add(taxid);
			} catch (NumberFormatException e) {
				GenomeParser.Taxonomony taxid = genomeParser
						.getTaxonomony(speciesName);
				if (taxid == null) {
					taxid = genomeParser.getTaxonomonyByUniqueName(speciesName);
					if (taxid == null)
						System.err
								.println("Can not identify species name in kegg: "
										+ speciesName);
					else
						orgs.add(taxid);
				} else {
	...
	public ArgumentDefinition<?>[] getArgumentDefinitions() {
		...
		args
				.add(new FileArgumentDefinition(
						FileArgumentDefinition.INPUT_DIR,
						FileArgumentDefinition.INPUT_DIR_DESC, true, true,
						true, false));
		args.add(new SpeciesArgumentDefinition(SPECIES_ARG, SPECIES_ARG_DESC,
				true));
		args.add(new BooleanArgumentDefinition(IMPORT_SEQS_4_SPECIES_ARG,
				IMPORT_SEQS_4_SPECIES_ARG_DESC, true, false));
		args.add(new SpeciesArgumentDefinition(SPECIES_OTHO_ARG,
				SPECIES_OTHO_ARG_DESC, false));
	...
	private void setUpParameters() throws IOException,
		...
		Boolean seq = (Boolean) args
				.getUniqueValue(ArgumentNames.IMPORT_SEQS_4_SPECIES_ARG);
	...
	public void start() throws Exception {
		...
		File inputDir = new File((String) args
		...
		FileIndex pathwayResource = fr
				.getIndex(FileRegistry.DataResource.PATHWAY);
		FileIndex ligandResource = fr
				.getIndex(FileRegistry.DataResource.LIGAND);
		FileIndex briteResource = fr.getIndex(FileRegistry.DataResource.BRITE);
		FileIndex medicusResource = fr
				.getIndex(FileRegistry.DataResource.MEDICUS);

		GenomeParser genomeParser = new GenomeParser(genesResource
				.getFile("genome"));

		Set<GenomeParser.Taxonomony> orgs = parseTaxids((List<String>) args
				.getObjectValueList(ArgumentNames.SPECIES_ARG), genomeParser);

		Set<GenomeParser.Taxonomony> orthologOrgs = parseTaxids(
				(List<String>) args
						.getObjectValueList(ArgumentNames.SPECIES_OTHO_ARG),
				genomeParser);
		...
		final DPLPersistantSet<Pathway> pathwayCache = new DPLPersistantSet<Pathway>(
				env, Pathway.class);
		DPLPersistantSet<net.sourceforge.ondex.parser.kegg56.sink.Relation> relationsCache = new DPLPersistantSet<net.sourceforge.ondex.parser.kegg56.sink.Relation>(
				env, net.sourceforge.ondex.parser.kegg56.sink.Relation.class);
		DPLPersistantSet<net.sourceforge.ondex.parser.kegg56.sink.Sequence> sequenceCache = new DPLPersistantSet<net.sourceforge.ondex.parser.kegg56.sink.Sequence>(
				env, net.sourceforge.ondex.parser.kegg56.sink.Sequence.class);
		...
		/*
		 * This needs updating to the latest version, whenever KEGG changes its
		 * DTD file. There needs to be a better way for the future.
		 */
		URL url = new URL("http://www.genome.jp/kegg/xml/KGML_v0.7.1_.dtd");
		try {
			String file = File.createTempFile("KGML_v0.7.1_", ".dtd")
					.getAbsolutePath();
			saveURL(url, file);
			xmlParser = new XMLParser(new File(file).toURI().toURL());
			// System.out.println(url+" saved to "+new
			// File(file).toURI().toURL());
		} catch (IOException e) {
			xmlParser = new XMLParser();
			System.err.println("Unable to access " + url.toString()
					+ " disabling validation: " + e.getMessage());
		}

		Set<Future<?>> futures = new HashSet<Future<?>>();

		for (String kgmlFile : kgmlFiles)
			futures.add(EXECUTOR.submit(new KGMLJob(xmlParser, kgmlResource
					.getFile(kgmlFile))));
		...
		for (Future<?> future : futures) {
			try {
				...
				int percentComplete = Math
						.round(((float) kgmlFilesCompleted / (float) futures
								.size()) * 100f);

				if (percentComplete % 10 == 0
						&& percentComplete > percentProgress) {
					percentProgress = percentComplete;
					System.out.println("KGML pathways parsed "
							+ percentComplete + " % (" + kgmlFilesCompleted
							+ ")");
				}
		...
		if (DEBUG)
			System.out.println("ReactionLigandDBParser");
		...
		if (DEBUG)
			System.out.println("CompParser");
		CompParser compParser = new CompParser(ligandResource
				.getFile("compound"), ligandResource.getFile("glycan"),
				medicusResource.getFile("drug"));
		Map<String, Concept> compounds = compParser.parse();

		if (DEBUG)
			System.out.println("Processing ChemicalStructure");
		Pattern molRegex = Pattern.compile("^C[\\d]{5}\\.mol$");
		List<String> molFiles = ligandResource.getFileNames(molRegex, true);
		for (String filename : molFiles)
			compParser.parseMol(compounds, filename, ligandResource
					.getFile(filename));
		...
		GeneFilesParser geneParser = new GeneFilesParser(genesResource, merge,
				sequenceCache, relationsCache, pathwayCache);
		...
		new KoRelationMerger().mergeAndWrite(koPathwayMerger.getKo2Genes(),
				koParser.getKoAccessionToKoConcept());
	...
	private void saveURL(URL url, String filename) throws IOException {
		...
		connection.connect();
		InputStreamReader ReadIn = new InputStreamReader(connection
				.getInputStream());
	...
	private Set<String> getSpeciesToParse(Set<GenomeParser.Taxonomony> orgs,
			...
			if (keggId.equals("all")) {
				...
				for (String species : genomeParser.getAllKeggSpecies()) {
					if (species.equals("all"))
						continue;
					GenomeParser.Taxonomony taxon = genomeParser
							.getTaxonomony(species);
