 ...
 *
 */
package net.sourceforge.ondex.parser.kegg56.relation;
...
public class RelationPathwayParser {
	...
	public void parseAndWrite(DPLPersistantSet<Pathway> pathways,
		...
		for (Pathway pathway : cursor) {
			...
			for (net.sourceforge.ondex.parser.kegg56.data.Relation r : pathway
					.getRelations()) {
					...
					for (Subtype subtype : r.getSubtype()) {
						if (subtype.getName().equalsIgnoreCase("expression"))
							...
							processInteraction_MissingGErel(entry1, entry2);
								...
								"activation"))
							processActivated_byGErel(entry1, entry2);
						else
						...
						for (Subtype subtype : r.getSubtype()) {
							if (subtype.getName().equalsIgnoreCase("compound")) {
								...
								processStatechange_fromPCrel(entry1, entry2);
									...
									"methylation"))
								processMethylated_byPCrel(entry1, entry2);
							else
	...
	private void processActivated_byGErel(Entry entry1, Entry entry2) {
		...
		for (String entry1ID : entry1.getConceptIDs()) {
			...
			if (cw.conceptParserIDIsWritten(fromID)) {
				...
				for (String entry2ID : entry2.getConceptIDs()) {
					...
					String toID = entry2ID.trim().toUpperCase();
					if (entry2.getType().equals("gene"))
						toID = stripEnding(toID) + "_GE";
					if (cw.conceptParserIDIsWritten(toID)) {

						// ac_by between gene and compound
						Relation ac_by = new Relation(toID, fromID,
								MetaData.RT_ACTIVATED_BY);
						if (relationsCache.contains(ac_by.pk)) {
							ac_by = relationsCache.get(ac_by.pk);
						}
						...
						System.out.println(toID
								+ " not found (to). processActivated_byGErel");
					}
				}
			} else {
				System.out.println(fromID
						+ " not found (from). processActivated_byGErel");
			}
		}
	}
			...
			if (cw.conceptParserIDIsWritten(fromID)) {
				...
				for (String entry2ID : entry2.getConceptIDs()) {
					...
					if (cw.conceptParserIDIsWritten(toID)) {
						...
						Relation dm_by = new Relation(toID, fromID,
			...
			if (cw.conceptParserIDIsWritten(fromID)) {
				...
				for (String entry2ID : entry2.getConceptIDs()) {
					...
					if (cw.conceptParserIDIsWritten(toID)) {
						...
						Relation de_by = new Relation(toID, fromID,
			...
			if (cw.conceptParserIDIsWritten(fromID)) {
				...
				for (String entry2ID : entry2.getConceptIDs()) {
					...
					if (cw.conceptParserIDIsWritten(toID)) {
						...
						Relation ex_by = new Relation(toID, fromID,
			...
			if (cw.conceptParserIDIsWritten(fromID)) {
				...
				for (String entry2ID : entry2.getConceptIDs()) {
					...
					if (cw.conceptParserIDIsWritten(toID)) {
						...
						Relation id_by = new Relation(toID, fromID,
			...
			if (cw.conceptParserIDIsWritten(fromID)) {
				...
				for (String entry2ID : entry2.getConceptIDs()) {
					...
					if (cw.conceptParserIDIsWritten(toID)) {
						...
						Relation in_by = new Relation(toID, fromID,
	...
	private void processMethylated_byPCrel(Entry entry1, Entry entry2) {
			...
			if (cw.conceptParserIDIsWritten(fromID)) {
				...
				for (String entry2ID : entry2.getConceptIDs()) {
					...
					if (cw.conceptParserIDIsWritten(toID)) {
						...
						Relation me_by = new Relation(toID, fromID,
						...
						System.out.println(toID
								+ " not found (to). processMethylated_byPCrel");
					}
				}
			} else {
				System.out.println(fromID
						+ " not found (from). processMethylated_byPCrel");
			}
		}
	}
			...
			if (cw.conceptParserIDIsWritten(fromID)) {
				...
				for (String entry2ID : entry2.getConceptIDs()) {
					...
					if (cw.conceptParserIDIsWritten(toID)) {
						...
						Relation ph_by = new Relation(toID, fromID,
								MetaData.RT_PHOSPHORYLATED_BY);
						if (relationsCache.contains(ph_by.pk)) {
							ph_by = relationsCache.get(ph_by.pk);
						}
